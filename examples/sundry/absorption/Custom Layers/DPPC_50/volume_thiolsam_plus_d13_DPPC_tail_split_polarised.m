function [output,sub_rough] = volume_thiolsam_plus_d13_DPPC_tail_split_polarised(params,bulk_in,bulk_out,contrast)
...
Cw_thick = params(12);
Bilayer_APM = params(13);
Bilayer_wph = params(14);
Bilayer_rough = params(15);
Bilayer_coverage = params(16);
...
SAMTAILS = [thickTail sldTail Gold_Rough];
SAMHEAD = [thickHead sldHead Gold_Rough];
...
sumb_Head = dCHOL + PO4 + dGLYC + 2*COO;
...
BILTAILS = [thickTail sldTail Bilayer_rough];
BILHEAD = [thickHead sldHead Bilayer_rough];
BILME = [thickMe sldMe Bilayer_rough];

BILAYER = [BILHEAD ; BILTAILS ; BILME; BILME ; BILTAILS ; BILHEAD];

CW = [Cw_thick bulk_out(contrast) Bilayer_rough];
