classdef layersClassComplexSLD < handle
    ...
    methods
        ...
        function obj = layersClassRealSLD()
            ...
            varNames = {'Name','Thickness','SLD_re','SLD_im','Roughness','Hydration','Hydrate with'};
        ...
        function obj = addLayer(obj, paramNames, varargin)
            ...
            elseif length(layerDetails) == 1 && ischar(layerDetails{1})
                ...
                if length(layerDetails) == 5
                    % No hydration
                    layerDetails = {layerDetails{1},layerDetails{2},layerDetails{3},layerDetails{4},layerDetails{5},NaN,hydrationTypes.BulkIn.value};
                elseif length(layerDetails) ~= 7
                    throw(invalidNumberOfInputs('Incorrect number of parameters for layer definition. Either 0, 1, 5, or 7 inputs are required.'));
                end
                ...
                %  . . . (unless p=6 which can also be Nan)
                if isnan(layerDetails{6})
                    newRow{6} = NaN;
                else
                    newRow{6} = obj.findParameter(layerDetails{6}, paramNames);
                end
        ...
        function obj = setLayerValue(obj, row, col, inputValue, paramNames)
            ...
            if ~isnumeric(col) || col < 2  || col > 7
                throw(indexOutOfRange('Column index should be a number between 2 and 7'));
            end
